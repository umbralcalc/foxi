...
class foxi:
    ...
    def __init__(self, path_to_foxi_directory):
        self.path_to_foxi_directory = (
            path_to_foxi_directory  # Set this upon declaration
        )
        ...
        self.number_of_forecast_samples = (
            1000  # Number of samples to take from the forecast distribution
        )
        self.flat_function
        self.exp_function
        self.power10_function
        self.dictionary_of_column_types = {
            "flat": self.flat_function,
            "log": self.exp_function,
            "log10": self.power10_function,
        }

    def set_chains(
        self,
        name_of_chains,
        chains_column_numbers,
        number_of_points,
        weights_column=None,
        column_types=None,
    ):
        ...
        if column_types:
            ...
            self.column_types_are_set = (
                True  # All columns are as input unless this is True
            )

        running_total = (
            0  # Initialize a running total of points read in from the chains
        )

        with open(
            self.path_to_foxi_directory
            + "/"
            + self.chains_directory
            + self.current_data_chains
        ) as file:
            ...
            for line in file:
                ...
                if self.chains_weights_are_set == True:
                    self.chains_weights.append(
                        columns[self.current_data_chains_weights_column]
                    )
                else:
                ...
                if self.column_types_are_set == True:
                    fiducial_point_vector = self.column_functions(
                        np.asarray(columns[chains_column_numbers])
                    )
                else:
                    fiducial_point_vector = np.asarray(
                        columns[chains_column_numbers]
                    )  # All columns are flat formats unless this is True

                self.chains_data.append(fiducial_point_vector)

                running_total += 1  # Also add to the running total
                if running_total >= number_of_points:
                    break  # Finish once reached specified number of data points
    ...
    def set_model_name_list(
        self,
        model_name_list,
        prior_column_numbers,
        number_of_prior_points,
        prior_column_types=None,
    ):
        ...
        if prior_column_types:
            ...
            self.prior_column_types_are_set = (
                True  # All columns are as input unless this is True
            )
        ...
        for i in range(0, len(self.model_name_list)):
            ...
            running_total = (
                0  # Initialize a running total of points read in from each prior
            )

            with open(
                self.path_to_foxi_directory
                + "/"
                + self.priors_directory
                + self.model_name_list[i]
            ) as file:
                ...
                for line in file:
                    ...
                    if self.prior_column_types_are_set == True:
                        prior_point_vector = self.column_functions(
                            np.asarray(columns[prior_column_numbers[i]]), prior=True
                        )
                    else:
                        prior_point_vector = np.asarray(
                            columns[prior_column_numbers[i]]
                        )  # All columns are as input unless this is True

                    self.prior_data[i].append(
                        np.asarray(prior_point_vector)
                    )  # Store the data for the KDE

                    # Calculate the forecast probability and therefore the contribution to the Bayesian evidence for each model
                    running_total += 1  # Also add to the running total
                    if running_total >= number_of_prior_points:
                        break  # Finish once reached specified number of prior points

            c_string = ""
            for j in range(0, len(prior_column_numbers[i])):
                c_string += "c"
            # Silly notational detail to get the kde to work for continuous variables in all dimensions

            self.density_functions.append(
                kde(self.prior_data[i], var_type=c_string)
            )  # Append a new Density Estimation function to the list for use later
    ...
    def column_functions(self, input_value, prior=False):
        ...
        if prior == False:
            return [
                self.dictionary_of_column_types[self.column_types[i]](input_value[i])
                for i in range(0, len(self.column_types))
            ]
        else:
            return [
                self.dictionary_of_column_types[self.prior_column_types[i]](
                    input_value[i]
                )
                for i in range(0, len(self.prior_column_types))
            ]
    ...
    def analyse_foxiplot_line(self, line_1, as_a_function=False):
        ...
        if as_a_function == False:
            if self.mix_models_was_used == False:
                print(
                    "Just read in an ordinary foxiplot data file with data dimension "
                    + str(dim_data)
                    + " and the number of model pairs is "
                    + str(num_models)
                )
            if self.mix_models_was_used == True:
                print(
                    "Just read in a `mix_models' foxiplot data file with data dimension "
                    + str(dim_data)
                    + " and the number of model pairs is "
                    + str(num_models)
                )
    ...
    def decide_on_best_computation(
        self,
        ML_point,
        samples_model_ML,
        kde_model_ML,
        samples_model_evidence,
        error_vector,
        ML_threshold,
        fiducial_point_vector,
        forecast_data_function,
        model_index,
    ):
            ...
            if ML_point * np.exp(-ML_threshold) < samples_model_ML:
                ...
                if any(
                    ...
                    for value in np.asarray(
                        np.sqrt(self.density_functions[model_index].bw)
                    )
                    - np.asarray(error_vector)
                ):
                    use_sampling = False
                ...
                if use_sampling == False:
                    ...
                    model_evidence = np.sum(
                        self.density_functions[model_index].pdf(
                            forecast_data_function(
                                fiducial_point_vector,
                                fiducial_point_vector,
                                error_vector,
                                samples=self.number_of_forecast_samples,
                            )
                        )
                    ) / float(self.number_of_forecast_samples)
    ...
    def decide_on_best_computation_with_fisher(
        self,
        ML_point,
        samples_model_ML,
        kde_model_ML,
    ...
    def utility_functions(
        ...
        ML_point = forecast_data_function(
            fiducial_point_vector, fiducial_point_vector, error_vector
        )
        ...
        forecast_data_function_normalisation = np.sum(
            np.asarray(self.chains_weights)
            * forecast_data_function(
                np.asarray(self.chains_data), fiducial_point_vector, error_vector
            )
        )
        # Compute the future posterior normalisation for the dkl value

        logargument = np.sum(np.asarray(self.chains_weights)) * np.asarray(
            forecast_data_function(
                np.asarray(self.chains_data), fiducial_point_vector, error_vector
            )
            / forecast_data_function_normalisation
        )
        ...
        for i in range(0, len(self.model_name_list)):
            ...
            kde_model_ML = self.density_functions[i].pdf(
                fiducial_point_vector
            ) * forecast_data_function(
                fiducial_point_vector, fiducial_point_vector, error_vector
            )
            ...
            E[i] = np.sum(
                forecast_data_function(
                    np.asarray(self.prior_data[i]), fiducial_point_vector, error_vector
                )
                / float(number_of_prior_points)
            )
            # Calculate the forecast probability and therefore the contribution to the Bayesian evidence for each model

            samples_model_ML = np.max(
                forecast_data_function(
                    np.asarray(self.prior_data[i]), fiducial_point_vector, error_vector
                )
            )
            ...
            [model_ML, E[i]] = self.decide_on_best_computation(
                ML_point,
                samples_model_ML,
                kde_model_ML,
                E[i],
            ...
            if ML_point * np.exp(-ML_threshold) < model_ML:
                model_valid_ML[i] = 1.0
            ...
            if mix_models == False:
                ...
                if E[j] == 0.0:
                    ...
                    if E[0] == 0.0:
                        ...
                        abslnB[
                            j
                        ] = 1000.0  # Maximal permitted value for an individual sample
                if E[0] == 0.0:
                    if E[j] == 0.0:
                        abslnB[j] = 0.0
                    else:
                    ...
                    abslnB[j] = abs(
                        np.log(E[j]) - np.log(E[0])
                    )  # Compute absolute log Bayes factor utility
                ...
                if abs(np.log(E[j]) - np.log(E[0])) >= 5.0:
                    if j > 0:
                        decisivity[
                            j
                        ] = 1.0  # Compute decisivity utility (for each new forecast distribution this is either 1 or 0)
            ...
            if mix_models == True:
                ...
                for k in range(avoid_repeat, len(self.model_name_list)):
                    ...
                    if E[k] == 0.0:
                        ...
                        abslnB[count_elements] = abs(
                            np.log(E[k]) - np.log(E[j])
                        )  # Compute absolute log Bayes factor utility
                    # The block above deals with unruly values like 0.0 inside the logarithms
    ...
    def gaussian_forecast(
        self, prior_point_vector, fiducial_point_vector, error_vector, samples=None
    ):
        ...
        if samples:
            ...
            return sps.multivariate_normal(fiducial_point_vector, covmat).pdf(
                prior_point_vector
            )
            # Return a density if sample number is not chosen

    def fisher_matrix_forecast(
        self, prior_point_vector, fiducial_point_vector, fisher_matrix, samples=None
    ):
        # Likelihood at some point - prior_point_vector - in arbirary dimensions using a Gaussian
    ...
    def fisher_utility_functions(
        ...
        ML_point = forecast_data_function(
            fiducial_point_vector, fiducial_point_vector, fisher_matrix
        )
        # The maximum Maximum Likelihood point is trivial to find
        DKL = 0.0
        # Initialise the integral count Kullback-Leibler divergence at 0.0
        ...
        forecast_data_function_normalisation = np.sum(
            np.asarray(self.chains_weights)
            * forecast_data_function(
                np.asarray(self.chains_data), fiducial_point_vector, fisher_matrix
            )
        )
        # Compute the future posterior normalisation for the dkl value
        ...
        for i in range(0, len(self.model_name_list)):
            ...
            E[i] = np.sum(
                forecast_data_function(
                    np.asarray(self.prior_data[i]), fiducial_point_vector, fisher_matrix
                )
                / float(number_of_prior_points)
            )
            # Calculate the forecast probability and therefore the contribution to the Bayesian evidence for each model
            ...
            [model_ML, E[i]] = self.decide_on_best_computation_with_fisher(
                ML_point,
                samples_model_ML,
                kde_model_ML,
                E[i],
    ...
    def run_foxi(self, error_vector, mix_models=False):
        ...
        forecast_data_function = (
            self.gaussian_forecast
        )  # Change the forecast data function here (if desired)
        ...
        if mix_models == False:
            ...
            plot_data_file.write(
                str(len(self.chains_column_numbers))
                + "\t"
                + str(len(self.model_name_list))
                + "\t"
                + str(0)
                + "\n"
            )
        ...
        if mix_models == True:
            ...
            plot_data_file = open(
                ...
                + "/"
                + self.output_directory
                + "foxiplots_data_mix_models.txt",
                "w",
            )
            plot_data_file.write(
                str(len(self.chains_column_numbers))
                + "\t"
                + str(possible_model_pairs)
                + "\t"
                + str(1)
                + "\n"
            )
        ...
        for j in range(0, len(self.chains_data)):
            ...
            [abslnB, deci, DKL, lnE, valid_ML] = self.utility_functions(
                self.chains_data[j],
                self.chains_column_numbers,
                self.prior_column_numbers,
                forecast_data_function,
                self.number_of_points,
                self.number_of_prior_points,
                error_vector,
                mix_models=mix_models,
            )

            plot_data_file.write(
                str(self.chains_weights[j])
                + "\t"
                + "\t".join(map(str, np.asarray(self.chains_data[j])))
                + "\t"
                + str(j)
                + "\t"
                + "\t".join(map(str, np.asarray(abslnB)))
                + "\t"
                + str(j)
                + "\t"
                + "\t".join(map(str, np.asarray(lnE)))
                + "\t"
                + str(j)
                + "\t"
                + "\t".join(map(str, np.asarray(valid_ML)))
                + "\t"
                + str(j)
                + "\t"
                + str(DKL)
        ...
        points_category_file = open(
            self.path_to_foxi_directory
            + "/"
            + self.output_directory
            + "foxiplots_data_number_points_categories.txt",
            "w",
        )
        # Initialize output file containing the counts of each category of point (defined in arxiv:1803.09491)

        points_category_file.write(
            "Number of points in each category (defined in arxiv:1803.09491) for each model:"
            + "\n"
        )
        for i in range(0, len(self.model_name_list)):
            points_category_file.write(
                ...
                + ": "
                + "n_A = "
                + str(self.number_of_category_A_points[i])
                + " "
                + "n_B = "
                + str(self.number_of_category_B_points[i])
                + " "
                + "n_C = "
                + str(self.number_of_category_C_points[i])
                + " "
                + "n_D = "
                + str(self.number_of_category_D_points[i])
                + "\n"
            )
            # Write output
    ...
    def run_foxifish(self, mix_models=False):
        ...
        for j in range(0, len(self.chains_data)):
            ...
            [abslnB, deci, DKL, lnE, valid_ML] = self.fisher_utility_functions(
                self.chains_data[j],
                self.chains_column_numbers,
                self.prior_column_numbers,
                forecast_data_function,
                self.number_of_points,
                self.number_of_prior_points,
                self.fisher_matrix_function(self.chains_data[j]),
                mix_models=mix_models,
            )
    ...
    def rerun_foxi(
        self,
        foxiplot_samples,
        number_of_foxiplot_samples,
        predictive_prior_types,
        TeX_output=False,
        model_name_TeX_input=[],
        as_a_function=False,
    ):
        ...
        with open(
            self.path_to_foxi_directory + "/" + self.output_directory + foxiplot_samples
        ) as file:
        ...
        [
            number_of_fiducial_point_dimensions,
            number_of_models,
        ] = self.analyse_foxiplot_line(line_for_analysis_1, as_a_function=as_a_function)
        ...
        running_total = (
            -1
        )  # Initialize a running total of points read in from the foxiplot data file
        ...
        with open(
            ...
            for line in file:
                if running_total != -1:
                    ...
                    chains_point_weight = float(
                        columns[0]
                    )  # Find the weight to the point in the chains
                    for j in range(0, len(columns)):
                        if j < number_of_fiducial_point_dimensions + 1 and j > 0:
                            fiducial_point_vector.append(float(columns[j]))
                        if (
                            j > number_of_fiducial_point_dimensions + 1
                            and j
                            < number_of_fiducial_point_dimensions + 1 + number_of_models
                        ):
                            if (
                                np.isnan(float(columns[j])) == False
                                and np.isfinite(float(columns[j])) == True
                            ):
                                abslnB[
                                    j - number_of_fiducial_point_dimensions - 2
                                ] = float(columns[j])
                            else:
                                abslnB[
                                    j - number_of_fiducial_point_dimensions - 2
                        ...
                        if (
                            ...
                            if float(columns[j]) == 0.0:
                                invalid_ML.append(1.0)
                            if float(columns[j]) == 1.0:
                                invalid_ML.append(0.0)
                    ...
                    predictive_prior_weight = (
                        chains_point_weight  # Initialize the prior weight unit value
                    )
                    ...
                    expected_abslnB = expected_abslnB + (
                        abslnB * predictive_prior_weight
                    )
                    decisivity_ML = decisivity_ML + (
                        valid_ML * predictive_prior_weight * deci
                    )
                    expected_abslnB_ML = expected_abslnB_ML + (
                        valid_ML * predictive_prior_weight * abslnB
                    )  # Do quick vector additions to update the expected utilities
                    expected_DKL += DKL * predictive_prior_weight

                    proportion_of_ML_failures = proportion_of_ML_failures + (
                        invalid_ML * predictive_prior_weight
                    )
                ...
                running_total += 1  # Also add to the running total
                if running_total >= number_of_foxiplot_samples:
                    break  # Finish once reached specified number of data points
        ...
        with open(
            ...
            for line in file:
                if running_total != -1:
                    ...
                    predictive_prior_weights = [
                        predictive_prior_weight for k in range(0, number_of_models)
                    ]
                    ...
                    valid_ML = np.asarray(valid_ML)
                    som_abslnB += (
                        ((abslnB - expected_abslnB) ** 2)
                        * predictive_prior_weights
                        / predictive_prior_weight_total
                    )
                    som_abslnB_ML += (
                        ((abslnB - expected_abslnB_ML) ** 2)
                        * predictive_prior_weights
                        * (valid_ML / total_valid_ML)
                    )  # Do quick vector additions to update second-moment utilities
                    som_DKL += (
                        ((DKL - expected_DKL) ** 2)
                        * predictive_prior_weight
                        / predictive_prior_weight_total
        ...
        if as_a_function == False:
            output_data_file = open(
                self.path_to_foxi_directory
                + "/"
                + self.output_directory
                + "foxiplots_data_summary.txt",
                "w",
            )
            output_data_file.write(
                " [<|lnB|>_1, <|lnB|>_2, ...] = " + str(expected_abslnB) + "\n"
            )
            output_data_file.write(" [DECI_1, DECI_2, ...] = " + str(decisivity) + "\n")
            output_data_file.write("<DKL> = " + str(expected_DKL) + "\n")
            output_data_file.write(
                " [<|lnB|_ML>_1, <|lnB|_ML>_2, ...] = " + str(expected_abslnB_ML) + "\n"
            )
            output_data_file.write(
                " [r_ML|_1, r_ML|_2, ...] = " + str(proportion_of_ML_failures) + "\n"
            )
            output_data_file.write(
                " [< (|lnB|_1 - <|lnB|>_1)^2 >, < (|lnB|_2 - <|lnB|>_2)^2 >, ...] = "
                + str(som_abslnB)
                + "\n"
            )
            output_data_file.write(
                " [< (|lnB|_1 - <|lnB|>_1_ML)^2 >_ML, < (|lnB|_2 - <|lnB|>_2_ML)^2 >_ML, ...] = "
                + str(som_abslnB_ML)
                + "\n"
            )
            output_data_file.write(
                " [DECI_1|_ML, DECI_2|_ML, ...] = " + str(decisivity_ML) + "\n"
            )
        ...
        if TeX_output == True:
            ...
            output_data_file = open(
                ...
                + "/"
                + self.output_directory
                + "foxiplots_data_summary_TeX.txt",
                "w",
            )
            ...
            output_data_file.write(
                r"\multicolumn{1}{c}{\cellcolor{red!55}} & \multicolumn{5}{|c|}{Data Name}  \\      \hline"
                + "\n"
            )
            output_data_file.write(
                r"\backslashbox{${\cal M}_\beta$ - ${\cal M}_\gamma$}{$\langle  U\rangle$}  & $\langle \vert \ln {\rm B}_{\beta \gamma}\vert \rangle$ & $\langle \vert \ln {\rm B}_{\beta \gamma} \vert \rangle_{{}_{\rm ML}}$ & $\mathscr{D}_{\beta \gamma}$ & $\mathscr{D}_{\beta \gamma}\vert_{{}_{\rm ML}}$ & $r_{{}_\mathrm{ML}}$ \\     \hline\hline"
                + "\n"
            )
            for k in range(0, number_of_models):
                if len(model_name_TeX_input) == 0:
                    if self.mix_models_was_used == False:
                        if round(np.sqrt(som_abslnB[k]), 2) < round(
                            expected_abslnB[k], 2
                        ):
                            mid_string_1 = (
                                str(round(expected_abslnB[k], 2))
                                + " $\pm$ "
                                + str(round(np.sqrt(som_abslnB[k]), 2))
                            )
                        else:
                            mid_string_1 = (
                                "$"
                        ...
                        if round(np.sqrt(som_abslnB_ML[k]), 2) < round(
                            expected_abslnB_ML[k], 2
                        ):
                            mid_string_2 = (
                                str(round(expected_abslnB_ML[k], 2))
                                + " $\pm$ "
                                + str(round(np.sqrt(som_abslnB_ML[k]), 2))
                            )
                        else:
                            mid_string_2 = (
                                "$"
                        ...
                        output_data_file.write(
                            ...
                            + mid_string_2
                            + " & "
                            + str(round(decisivity[k], 2))
                            + " & "
                            + str(round(decisivity_ML[k], 2))
                            + " & "
                            + str(round(proportion_of_ML_failures[k], 2))
                            + r"  \\      \hline"
                            + "\n"
                        )

                    if self.mix_models_was_used == True:
                        number_of_individual_models = (
                            int(0.5 + np.sqrt((2.0 * number_of_models) + 0.25)) - 1
                        )
                    ...
                    if self.mix_models_was_used == False:
                        ...
                        output_data_file.write(
                            model_name_TeX_input[k]
                            + " - "
                            + model_name_TeX_input[0]
                            + " & "
                            + mid_string_1
                            + " & "
                            + mid_string_2
                    ...
                    if self.mix_models_was_used == True:
                        ...
                        output_data_file.write(
                            model_name_TeX_input[first_in_order]
                            + " - "
                            + model_name_TeX_input[second_in_order]
                            + " & "
                            + mid_string_1
                            + " & "
                            + mid_string_2
    ...
    def plot_foxiplots(
        ...
        running_total = (
            -1
        )  # Initialize a running total of points read in from the data file

        with open(
            self.path_to_foxi_directory + "/" + self.output_directory + filename_choice
        ) as file:
            ...
            for line in file:
                ...
                running_total += 1  # Also add to the running total
                if running_total >= number_of_samples:
                    break  # Finish once reached specified number of data points
        ...
        if corner_plot == False:
            for i in range(0, len(column_numbers) - 1):
                ...
                y_sides = [
                    (y_step * float(j)) + ranges[i][0] for j in range(0, number_of_bins)
                ]
                x_sides = [
                    (x_step * float(j)) + ranges[i + 1][0]
                    for j in range(0, number_of_bins)
                ]
                # Generate list of bins

                z_values, x_sides, y_sides = np.histogram2d(
                    y_points, x_points, bins=(x_sides, y_sides)
                )
    ...
    def plot_convergence(
        ...
        for ns in range(1, number_of_bins):
            ...
            number_of_samples_to_plot = int(
                float(ns) * (float(total_number_of_samples) / float(number_of_bins))
            )
        ...
        for nm in range(0, len(np.asarray(tot_expected_abslnB).T)):
            ...
            axes[0, 1].plot(
                tot_number_of_samples, np.asarray(tot_expected_abslnB).T[nm]
            )
            axes[0, 1].set_title(
                r"$\langle \vert \ln {\rm B}_{\beta \gamma}\vert \rangle$",
            ...
            axes[0, 2].set_title(
                r"$\langle \vert \ln {\rm B}_{\beta \gamma} \vert \rangle_{{}_{\rm ML}}$",
                fontsize=self.fontsize,
            )
            ...
            axes[1, 0].plot(
                tot_number_of_samples, np.asarray(tot_proportion_of_ML_failures).T[nm]
            )
